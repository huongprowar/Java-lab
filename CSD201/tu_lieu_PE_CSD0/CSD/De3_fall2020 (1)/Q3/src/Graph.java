...
//        }
    ...
    void dijkstra2(int fro, int to, RandomAccessFile f) throws IOException {
        ...
        i = s.pop();
        int mm=listSelected.size() - 3/*<---Chinh sua so o day*/;//contains the last 3 vertices selected into the set S
        f.writeBytes(""+v[listSelected.get(mm)]+ "," + d[listSelected.get(mm)]);
        for (int m = listSelected.size() - 2/*<---Chinh sua so o day nhung -1*/; m < listSelected.size(); m++) {
